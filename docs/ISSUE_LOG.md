...
| 5 | Runtime selection | `pip`-installed `litert-lm` CLI has no way to target the connected Pixel 7a | The CLI installed via `pip` runs inference locally on the host machine only; Android execution requires a separately built, device-targeted binary | Switched to building `litert_lm_main` from source via Bazel with `--config=android_arm64`, using WSL2 + Android NDK r28b |
